    ...
    <Draggable draggableId={ticket.id} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          className={`group bg-dark-purple-800 border border-dark-purple-700 rounded-xl p-4 transition-all duration-200 hover:bg-dark-purple-700 ${
        ...
        >
          <Link to={`/tickets/${encodeId(ticket.id) || ticket.id}`} state={{ ticket }} className="block">
            ...
            <h3 className="text-text-primary font-semibold text-sm mb-2 line-clamp-2">{ticket.titulo}</h3>
            <p className="text-xs text-text-muted mb-3">{getTimeAgo(ticket.fechaCreacion)}</p>

            <div className="flex items-center justify-between mt-3 pt-3 border-t border-dark-purple-700/50">
              <span className="text-xs text-text-muted">{ticket.area}</span>
