...
export function EditarSucursalPage() {
  ...
  useEffect(() => {
    ...
    console.group('%c[EditarSucursalPage - DEBUG]', 'color: #9D84B7; font-weight: bold; font-size: 12px;');
    console.table({
    ...
    if (!id || id === "null" || id === "undefined") {
      console.error('%c[EditarSucursalPage] ❌ GUARDIA ACTIVADA - ID Inválido', 'color: #FF6B6B; font-weight: bold;', { id, encodedId });
      toast.error("El ID de la sucursal no es válido o ha expirado.");
      navigate("/sucursales");
      return;
    }

    console.log('%c[EditarSucursalPage] ✅ ID válido, llamando API con:', 'color: #51CF66; font-weight: bold;', { id, tipo: typeof id });

    async function loadSucursal() {
      try {
        console.log(`%c[EditarSucursalPage] 📡 GET /sucursales/${id}`, 'color: #4C6EF5; font-style: italic;');
        const data = await sucursalService.getById(id);
        if (!cancelled) {
          console.log('%c[EditarSucursalPage] ✅ Datos cargados correctamente', 'color: #51CF66;', { data });
          setSucursalData(data);
          setLoadError("");
        }
      } catch (error) {
        if (!cancelled) {
          console.error('%c[EditarSucursalPage] ❌ Error al cargar sucursal', 'color: #FF6B6B;', {
