...
export function UsuarioDetallePage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        console.log(`%c[UsuarioDetallePage] 📡 GET /usuarios/${id}`, 'color: #4C6EF5; font-style: italic;');
        const [userData, areaData] = await Promise.all([userService.getById(id), areaService.getAll()]);

        if (!cancelled) {
          console.log('%c[UsuarioDetallePage] ✅ Datos cargados correctamente', 'color: #51CF66;', { userData, areaCount: areaData.length });
          setUsuario(userData);
          setAreas(areaData);
          setLoadError("");
        }
        ...
        if (!cancelled) {
          console.error('%c[UsuarioDetallePage] ❌ Error al cargar usuario', 'color: #FF6B6B;', {
